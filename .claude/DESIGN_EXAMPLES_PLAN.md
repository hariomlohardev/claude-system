...
|---|---|---|---|
| Hero eyebrow | `DESIGN SYSTEM · VERSION 1.0` | `01 — Registry · Supabase live · v0.1.3` | Name registry + liveness + version |
| Hero h1 | `Claude System` | `Systems for <em>Claude Code</em>` | Product is Systems for Claude Code |
| Hero lede | quiet design system … via curl/npm/pip | `A curated registry — install a complete workflow in one shot. Fetched fresh from <code>/api/registry</code> (Supabase live).` | Must mention registry + fresh fetch + curl |
| Install tabs | win `install.ps1` + npm + pip / mac `install.sh` | Unify: primary `curl -fsSL https://claude-system-tau.vercel.app/install \| sh` for all OS + alt `npm i -g claude-system` / `pip install claude-system` | Our entrypoint is single `install.sh` at `/install`, no `install.ps1` |
| Systems rows | `01 Docs / 02 Create a system / 03 Get started` | `01 Browse → /browse` (live grid), `02 Create → /docs/creating-a-system`, `03 CLI → /docs/cli` · tags `browse / create / cli` | Our IA is browse/docs/cli not create.html |
| Activity | `a3f9c21 / 7b4e8d0 / e19ab44` placeholders | Live `git log --oneline -3` hashes+titles+dates, links to `hariomlohardev/claude-system/commits` | Must show real commits |
| Docs hub | Tokens/components/templates | `01 Guide / 02 Trust / 03 Contract / 04 CLI` → real `docs/creating-a-system.md` etc. | Docs are about Systems not tokens |
| CLI rows | 6 commands `init/new/serve/build/check/list` | 12 commands `list/search/info/install/remove/update/run/create/validate/report/doctor/completion` with real flags `--installed/--category/--all/ -- <args>` | Our CLI is the product |
...
## Sitemap we implement

```
/               → index.html (hero + install + systems rows + activity)
 /browse         → browse/index.html (from systems.html: search → /api/search?q= + pills ?category=)
 /system?name=<name> → system/index.html (new — row+manifest table+install block)
 /s/:name        → rewrite → system/index.html via vercel.json
 /docs           → docs/index.html (from docs.html hub)
 /docs/creating-a-system, /docs/security, /docs/spec, /docs/cli, /docs/contributing
 404.html
```
...
`public/style.css` (verify/replace tokens) → `public/app.js` (reveal/menu/copy/tabs/G + search) → `index.html` → `browse/index.html` → `system/index.html` → `docs/**` → `vercel.json` rewrites/headers → verify → push.

Plan is record — future AI reads this + `DESIGN.md` to rebuild.
